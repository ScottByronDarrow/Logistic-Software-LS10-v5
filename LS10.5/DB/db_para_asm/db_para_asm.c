...
|=====================================================================|
| $Id: db_para_asm.c,v 5.3 2002/07/18 06:24:14 scott Exp $
|  Program Name  : (db_para_asm.c)
...
|
| Revision 5.2  2002/02/04 03:57:38  scott
| ?
|
| Revision 5.1  2001/12/07 03:55:58  scott
| Updated to convert to app.schema and perform a general code clean.
|
=====================================================================*/
#define	CCMAIN
char	*PNAME = "$RCSfile: db_para_asm.c,v $",
		*PROG_VERSION = "@(#) - $Header: /usr/LS10/REPOSITORY/LS10.5/DB/db_para_asm/db_para_asm.c,v 5.3 2002/07/18 06:24:14 scott Exp $";
...
void 	CloseDB 			(void);
int 	spec_valid 			(int);
int 	DeletePline 		(void);
int 	InsertPline 		(void);
int 	LoadDbpd 			(long);
int 	UpdateData 			(void);
void 	SrchDblh 			(char *);
void 	SrchDbph 			(char *);
	...
	long	dbphHash)
{
	...
	dbpd_rec.dbph_hash = dbphHash;
	dbpd_rec.line_no = 0;  

	cc = find_rec (dbpd, &dbpd_rec, GTEQ, "r");
	while (!cc && dbpd_rec.dbph_hash == dbphHash)
	{
		fflush (stdout);
...
UpdateData (void)
{
    ...
    for (wk_line = i; wk_line < MAXLINES; wk_line++)
    {
	   	dbpd_rec.dbph_hash = dbph_rec.dbph_hash;
	   	dbpd_rec.line_no   = wk_line;
	   	if ( (!find_rec (dbpd, &dbpd_rec, COMPARISON, "r")))
			abc_delete (dbpd);
	   	else
		 	break;
	}
...
{
	work_open ();
	save_rec ("#Cd","#Paragraph Description");
	strcpy (dblh_rec.co_no, comm_rec.co_no);
	...
	if (cc)
		return;
	strcpy (dblh_rec.co_no, comm_rec.co_no);
	sprintf (dblh_rec.par_code, "C-%-8.8s", temp_str);
	cc = find_rec (dblh, &dblh_rec, COMPARISON, "r");
	if (cc)
...
{
	work_open ();
	save_rec ("#Cd","#Letter Description");
	strcpy (dbph_rec.co_no, comm_rec.co_no);
