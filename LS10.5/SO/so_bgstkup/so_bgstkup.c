...
|=====================================================================|
| $Id: so_bgstkup.c,v 5.6 2002/01/09 01:12:10 scott Exp $
|  Program Name  : (so_bgstkup.c                                      |
|  Program Desc  : (Update Stock From Customer Invoice File.       )   |
|---------------------------------------------------------------------|
|  Date Written  : 03/11/86        |  Author     : Scott Darrow       |
|---------------------------------------------------------------------|
| $Log: so_bgstkup.c,v $
| Revision 5.6  2002/01/09 01:12:10  scott
...
|
| Revision 5.5  2001/11/08 08:39:24  scott
| Updated to process as per batch control if number plates are active.
|
| Revision 5.4  2001/08/31 01:34:00  cha
| Updated to reflect Scott's changes to make sure that inla is correctly handled.
|
| Revision 5.3  2001/08/09 09:20:46  scott
| Updated to add FinishProgram () function
|
| Revision 5.2  2001/08/06 23:50:54  scott
| RELEASE 5.0
|
...
=====================================================================*/
#define	CCMAIN
char	*PNAME = "$RCSfile: so_bgstkup.c,v $",
		*PROG_VERSION = "@(#) - $Header: /usr/LS10/REPOSITORY/LS10.5/SO/so_bgstkup/so_bgstkup.c,v 5.6 2002/01/09 01:12:10 scott Exp $";
...
#define	LCL_VALID	(!strcmp (sobg2_rec.type, "SU"))
#define	BAD_HASH 	(sobg2_rec.hash <= 0L)
 ...
 char * argv [])
{
	...
	sprintf (programName, 		"%-14.14s", argv [0]);
	sprintf (findStatusFlag, 	"%-1.1s", argv [1]);
	sprintf (updateStatusFlag, 	"%-1.1s", argv [2]);

	cc = find_rec (bpro, &bpro_rec, GTEQ, "u");
	while (!cc && !strcmp (bpro_rec.program, programName))
	{
		abc_unlock (bpro);
		if (bpro_rec.pid != 0 && bpro_rec.pid == processID)
		{
			recordFound = 1;
			break;
		}
	...
	if (!recordFound)
	{
		...
		strcpy (bpro_rec.program, programName);
		bpro_rec.hash = 0L;
		bpro_rec.pid = 0;
	}
	abc_fclose (bpro);
...
OpenDB (void)
{
	...
	open_rec (sobg2, sobg_list,  SOBG_NO_FIELDS,  "sobg_id_no_2");
 ...
 void)
{
	...
	while (!prog_exit)
	{
		memset (&sobg2_rec, 0, sizeof (sobg2_rec));

		/*----------------------------
		| Initialise to bpro record. |
		----------------------------*/
		strcpy (sobg2_rec.type, "SU");
		sobg2_rec.lpno = -1;
		sobg2_rec.hash = 0L;
		...
		while (!prog_exit && !cc)
		{
			...
			/*-----------------------------------
			| Set current PID from sobg record. |
			-----------------------------------*/
			currentProcessID = sobg2_rec.pid;

			/*-----------------------------------------------
			| Creat A P/Slip (cohr/coln) from (sohr/soln)	|
			-----------------------------------------------*/
			backgroundHash = sobg2_rec.hash;
			...
			/*-----------------------------------
			| Process Invoice to Stock. 		|
			-----------------------------------*/
			ProcessHeader (backgroundHash);

			/*-----------------------------------------------
			| Recalc all records not finished stock update. |
			-----------------------------------------------*/
			recalc_sobg ();
	...
	long	hhcoHash)
{
	...
	/*------------------------------------------------
	| Find cohr record and if not found then return. |
	------------------------------------------------*/
	...
	/*-------------------------------------------------------
	| Check status passed and status on cohr,if not same	|
    | record is already updated to ignore.		         	|
	-------------------------------------------------------*/
	if (cohr_rec.stat_flag [0] != findStatusFlag [0] && 
		cohr_rec.stat_flag [0] != 'T')
	{
		abc_unlock (cohr);
		return;
	}
