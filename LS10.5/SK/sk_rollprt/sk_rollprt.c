...
|=====================================================================|
| $Id: sk_rollprt.c,v 5.5 2002/07/17 09:57:59 scott Exp $
|  Program Name  : (sk_rollprt.c)
|  Program Desc  : (Standard Cost Rollup Print Program) 
|---------------------------------------------------------------------|
...
|
| Revision 5.4  2001/12/13 09:32:50  scott
| Updated to convert to app.schema and perform a general code clean.
|
=====================================================================*/
#define	CCMAIN
char	*PNAME = "$RCSfile: sk_rollprt.c,v $",
		*PROG_VERSION = "@(#) - $Header: /usr/LS10/REPOSITORY/LS10.5/SK/sk_rollprt/sk_rollprt.c,v 5.5 2002/07/17 09:57:59 scott Exp $";
...
ProcStdRollup (void)
{
	...
	for (i = 0; i < rollupCnt; i++)
	{
		...
		if (inei_rec.last_bom == local_rec.bom_no &&
			inei_rec.last_rtg == local_rec.rtg_no)
	...
	float 	cnvFct)
{
	...
	if (levelNo == 1)
		...
		req_rec.mat_cost = materialCost;

	for (i = 0; i < 3; i ++)
	{
		materialCost += req_rec.s_labour [i];
		materialCost += req_rec.s_machine [i];
		materialCost += req_rec.s_qc_check [i];
		materialCost += req_rec.s_special [i];
		materialCost += req_rec.s_other [i];
		materialCost += req_rec.a_labour [i];
		materialCost += req_rec.a_machine [i];
		materialCost += req_rec.a_qc_check [i];
		materialCost += req_rec.a_special [i];
		materialCost += req_rec.a_other [i];
	}
...
InitStruct (void)
{
	...
	for (i = 0; i < 3; i ++)
	{
		req_rec.s_labour [i] 	= 0.00;
		req_rec.s_machine [i] 	= 0.00;
		req_rec.s_qc_check [i] 	= 0.00;
		req_rec.s_special [i] 	= 0.00;
		req_rec.s_other [i] 	= 0.00;
		req_rec.a_labour [i] 	= 0.00;
		req_rec.a_machine [i] 	= 0.00;
		req_rec.a_qc_check [i] 	= 0.00;
		req_rec.a_special [i] 	= 0.00;
		req_rec.a_other [i] 	= 0.00;
	}
}
 ...
 * of the manufactured item passed.                 
 * Returns the total resource cost for the item hash passed.  
 */
void
CalcResources (
	 long 	hhbrHash, 
	 float 	requiredQty)
{
	double	requiredCost [3];	/* 0 - setup cost 1 - run cost 2 - clean cost */
	double	requiredOverhead [3];
	float	runTime;
	...
	if (!cc)
	{
			...
			rgln_rec.hhgr_hash == rghr_rec.hhgr_hash)
		{
			 ...
			 */
			requiredCost [0] = (double) rgln_rec.setup * rgln_rec.rate;
			requiredCost [0] /= 60.00;
			requiredCost [0] *= (double) rgln_rec.qty_rsrc;
			requiredCost [1] = (double) runTime * rgln_rec.rate;
			requiredCost [1] /= 60.00;
			requiredCost [1] *= (double) rgln_rec.qty_rsrc;
			requiredCost [2] = (double) rgln_rec.clean * rgln_rec.rate;
			requiredCost [2] /= 60.00;
			requiredCost [2] *= (double) rgln_rec.qty_rsrc;

			/* 
			 * Total overhead costs for the required quantity. 
			 */
			requiredOverhead [0] = (double) rgln_rec.setup * rgln_rec.ovhd_var;
			requiredOverhead [0] /= 60.00;
			requiredOverhead [0] += (double) rgln_rec.ovhd_fix;
			requiredOverhead [0] *= (double) rgln_rec.qty_rsrc;
			requiredOverhead [1] = (double) runTime * rgln_rec.ovhd_var;
			requiredOverhead [1] /= 60.00;
			requiredOverhead [1] += (double) rgln_rec.ovhd_fix;
			requiredOverhead [1] *= (double) rgln_rec.qty_rsrc;
			requiredOverhead [2] = (double) rgln_rec.clean * rgln_rec.ovhd_var;
			requiredOverhead [2] /= 60.00;
			requiredOverhead [2] += (double) rgln_rec.ovhd_fix;
			requiredOverhead [2] *= (double) rgln_rec.qty_rsrc;
			...
			if (!cc)
			{
				...
				{
				case	'L': /* Labour */
					req_rec.s_labour [0] += DOLLARS (requiredCost [0]);
					req_rec.a_labour [0] += DOLLARS (requiredOverhead [0]);
					req_rec.s_labour [1] += DOLLARS (requiredCost [1]);
					req_rec.a_labour [1] += DOLLARS (requiredOverhead [1]);
					req_rec.s_labour [2] += DOLLARS (requiredCost [2]);
					req_rec.a_labour [2] += DOLLARS (requiredOverhead [2]);
					break;
				case	'M': /* Machine */
					req_rec.s_machine [0] += DOLLARS (requiredCost [0]);
					req_rec.a_machine [0] += DOLLARS (requiredOverhead [0]);
					req_rec.s_machine [1] += DOLLARS (requiredCost [1]);
					req_rec.a_machine [1] += DOLLARS (requiredOverhead [1]);
					req_rec.s_machine [2] += DOLLARS (requiredCost [2]);
					req_rec.a_machine [2] += DOLLARS (requiredOverhead [2]);
					break;
				case	'Q': /* QC-Check */
					req_rec.s_qc_check [0] += DOLLARS (requiredCost [0]);
					req_rec.a_qc_check [0] += DOLLARS (requiredOverhead [0]);
					req_rec.s_qc_check [1] += DOLLARS (requiredCost [1]);
					req_rec.a_qc_check [1] += DOLLARS (requiredOverhead [1]);
					req_rec.s_qc_check [2] += DOLLARS (requiredCost [2]);
					req_rec.a_qc_check [2] += DOLLARS (requiredOverhead [2]);
					break;
				case	'S': /* Special */
					req_rec.s_special [0] += DOLLARS (requiredCost [0]);
					req_rec.a_special [0] += DOLLARS (requiredOverhead [0]);
					req_rec.s_special [1] += DOLLARS (requiredCost [1]);
					req_rec.a_special [1] += DOLLARS (requiredOverhead [1]);
					req_rec.s_special [2] += DOLLARS (requiredCost [2]);
					req_rec.a_special [2] += DOLLARS (requiredOverhead [2]);
					break;
				case	'O': /* Other */
					req_rec.s_other [0] += DOLLARS (requiredCost [0]);
					req_rec.a_other [0] += DOLLARS (requiredOverhead [0]);
					req_rec.s_other [1] += DOLLARS (requiredCost [1]);
					req_rec.a_other [1] += DOLLARS (requiredOverhead [1]);
					req_rec.s_other [2] += DOLLARS (requiredCost [2]);
					req_rec.a_other [2] += DOLLARS (requiredOverhead [2]);
					break;
				}
			}
	...
	double	materialCost)
{
	 ...
	 * Total setup costs 
	 */
	totalSetup 	+= req_rec.a_labour 	[0]	+ req_rec.a_machine 	[0]
				+ req_rec.a_qc_check 	[0]	+ req_rec.a_special 	[0]
				+ req_rec.a_other 		[0]	+ req_rec.s_labour 		[0]
				+ req_rec.s_machine 	[0]	+ req_rec.s_qc_check 	[0]
				+ req_rec.s_special 	[0]	+ req_rec.s_other 		[0];

	/* 
	 * Total run costs 
	 */
	totalRun 	+= req_rec.a_labour 	[1] + req_rec.a_machine 	[1]
				+ req_rec.a_qc_check 	[1] + req_rec.a_special 	[1]
				+ req_rec.a_other 		[1] + req_rec.s_labour 		[1]
				+ req_rec.s_machine 	[1] + req_rec.s_qc_check 	[1]
				+ req_rec.s_special 	[1] + req_rec.s_other 		[1];

	/* total clean costs */
	totalClean 	+= req_rec.a_labour 	[2] + req_rec.a_machine 	[2]
				+ req_rec.a_qc_check 	[2] + req_rec.a_special 	[2]
				+ req_rec.a_other 		[2] + req_rec.s_labour 		[2]
				+ req_rec.s_machine 	[2] + req_rec.s_qc_check 	[2]
				+ req_rec.s_special 	[2] + req_rec.s_other 		[2];

	totalCost  = req_rec.mat_cost;
	...
	fprintf (fout, "|%10.2f",		inei_rec.std_cost);
	fprintf (fout, "|%10.2f",		materialCost);
	fprintf (fout, "|%10.2f",		req_rec.mat_cost);
...
PrintTotal (void)
{
	...
	for (i = 0; i < 6; i ++)
	{
		inem_rec [i].hhbr_hash			= inei_rec.hhbr_hash;
		strcpy (inem_rec [i].est_no,	inei_rec.est_no);
		if (i == MATERIAL)
		...
		if (find_rec (inem, &inem_rec [i], EQUAL, "r"))
		{
	 ...
	 */
	totalSetup =
		inem_rec [LABOUR].setup_std_dir   + inem_rec [LABOUR].setup_std_fix +
		inem_rec [MACHINE].setup_std_dir  + inem_rec [MACHINE].setup_std_fix +
		inem_rec [QC_CHECK].setup_std_dir + inem_rec [QC_CHECK].setup_std_fix +
		inem_rec [SPECIAL].setup_std_dir  + inem_rec [SPECIAL].setup_std_fix +
		inem_rec [OTHER].setup_std_dir    + inem_rec [OTHER].setup_std_fix;

	/* 
	 * Total run costs 
	 */
	totalRun =
		inem_rec [LABOUR].run_std_dir     + inem_rec [LABOUR].run_std_fix +
		inem_rec [MACHINE].run_std_dir    + inem_rec [MACHINE].run_std_fix +
		inem_rec [QC_CHECK].run_std_dir   + inem_rec [QC_CHECK].run_std_fix +
		inem_rec [SPECIAL].run_std_dir    + inem_rec [SPECIAL].run_std_fix +
		inem_rec [OTHER].run_std_dir      + inem_rec [OTHER].run_std_fix;

	/* 
	 * Total clean costs 
	 */
	totalClean =
		inem_rec [LABOUR].clean_std_dir   + inem_rec [LABOUR].clean_std_fix +
		inem_rec [MACHINE].clean_std_dir  + inem_rec [MACHINE].clean_std_fix +
		inem_rec [QC_CHECK].clean_std_dir + inem_rec [QC_CHECK].clean_std_fix +
		inem_rec [SPECIAL].clean_std_dir  + inem_rec [SPECIAL].clean_std_fix +
		inem_rec [OTHER].clean_std_dir    + inem_rec [OTHER].clean_std_fix;

	/* 
	 * Total direct costs 
	 */
	resourceDirect  =
		inem_rec [LABOUR].setup_std_dir   + inem_rec [MACHINE].setup_std_dir +
		inem_rec [QC_CHECK].setup_std_dir + inem_rec [SPECIAL].setup_std_dir +
		inem_rec [OTHER].setup_std_dir    + inem_rec [LABOUR].run_std_dir +
		inem_rec [MACHINE].run_std_dir    + inem_rec [QC_CHECK].run_std_dir +
		inem_rec [SPECIAL].run_std_dir    + inem_rec [OTHER].run_std_dir +
		inem_rec [LABOUR].clean_std_dir   + inem_rec [MACHINE].clean_std_dir +
		inem_rec [QC_CHECK].clean_std_dir + inem_rec [SPECIAL].clean_std_dir +
		inem_rec [OTHER].clean_std_dir;

	/* 
	 * Total fixed costs 
	 */
	resourceFixed  =
		inem_rec [LABOUR].setup_std_fix   + inem_rec [MACHINE].setup_std_fix +
		inem_rec [QC_CHECK].setup_std_fix + inem_rec [SPECIAL].setup_std_fix +
		inem_rec [OTHER].setup_std_fix    + inem_rec [LABOUR].run_std_fix +
		inem_rec [MACHINE].run_std_fix    + inem_rec [QC_CHECK].run_std_fix +
		inem_rec [SPECIAL].run_std_fix    + inem_rec [OTHER].run_std_fix +
		inem_rec [LABOUR].clean_std_fix   + inem_rec [MACHINE].clean_std_fix +
		inem_rec [QC_CHECK].clean_std_fix + inem_rec [SPECIAL].clean_std_fix +
		inem_rec [OTHER].clean_std_fix;

	totalBatch = totalSetup + totalRun + 
				 totalClean + inem_rec [MATERIAL].setup_std_dir;
	...
	fprintf (fout, "|%10s",			" ");
	fprintf (fout, "|%10s",			" ");
	fprintf (fout, "|%10.2f|\n",	inem_rec [MATERIAL].setup_std_dir);

	fprintf (fout, "| Setup Costs   %20s", " ");
	fprintf (fout, "|%10.2f",	inem_rec [LABOUR].setup_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [MACHINE].setup_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [QC_CHECK].setup_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [SPECIAL].setup_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [OTHER].setup_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [LABOUR].setup_std_fix);
	fprintf (fout, "|%10.2f",	inem_rec [MACHINE].setup_std_fix);
	fprintf (fout, "|%10.2f",	inem_rec [QC_CHECK].setup_std_fix);
	fprintf (fout, "|%10.2f",	inem_rec [SPECIAL].setup_std_fix);
	fprintf (fout, "|%10.2f",	inem_rec [OTHER].setup_std_fix);
	fprintf (fout, "|%10.2f|\n",totalSetup);

	fprintf (fout, "| Run Costs     %20s", " ");
	fprintf (fout, "|%10.2f",	inem_rec [LABOUR].run_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [MACHINE].run_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [QC_CHECK].run_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [SPECIAL].run_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [OTHER].run_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [LABOUR].run_std_fix);
	fprintf (fout, "|%10.2f",	inem_rec [MACHINE].run_std_fix);
	fprintf (fout, "|%10.2f",	inem_rec [QC_CHECK].run_std_fix);
	fprintf (fout, "|%10.2f",	inem_rec [SPECIAL].run_std_fix);
	fprintf (fout, "|%10.2f",	inem_rec [OTHER].run_std_fix);
	fprintf (fout, "|%10.2f|\n",totalRun);

	fprintf (fout, "| Clean Costs   %20s", " ");
	fprintf (fout, "|%10.2f",	inem_rec [LABOUR].clean_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [MACHINE].clean_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [QC_CHECK].clean_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [SPECIAL].clean_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [OTHER].clean_std_dir);
	fprintf (fout, "|%10.2f",	inem_rec [LABOUR].clean_std_fix);
	fprintf (fout, "|%10.2f",	inem_rec [MACHINE].clean_std_fix);
	fprintf (fout, "|%10.2f",	inem_rec [QC_CHECK].clean_std_fix);
	fprintf (fout, "|%10.2f",	inem_rec [SPECIAL].clean_std_fix);
	fprintf (fout, "|%10.2f",	inem_rec [OTHER].clean_std_fix);
	fprintf (fout, "|%10.2f|\n",totalClean);
	...
	fprintf (fout, "|%-40.40s                   %-15.15s", " ", " ");
	fprintf (fout, "| COST PER  PRICE CONV |");
	fprintf (fout, "%58s|\n", " ");

	fprintf (fout, "|%-40.40sMaterial           %-15.15s", " ", " ");
	fprintf (fout, "|      %10.2f      |", inem_rec [MATERIAL].setup_std_dir);
	fprintf (fout, "%58s|\n", " ");

	fprintf (fout, "|%-40.40sResource Direct    %-15.15s", " ", " ");
	fprintf (fout, "|      %10.2f      |", resourceDirect);
	fprintf (fout, "%58s|\n", " ");

	fprintf (fout, "|%-40.40sResource Fixed     %-15.15s", " ", " ");
	fprintf (fout, "|      %10.2f      |", resourceFixed);
	fprintf (fout, "%58s|\n", " ");
	...
	fprintf (fout, "|%-40.40sTotal Cost         %-15.15s", " ", " ");
	fprintf (fout, "|      %10.2f      |", totalBatch);
	fprintf (fout, "%58s|\n", " ");
...
InitOutput (void)
{
	dsp_screen (" Printing Standard Cost Rollup Report ",
			comm_rec.co_no, 
			comm_rec.co_name);
...
{
	if (DETAIL)
		fprintf (fout, ".E STANDARD COST ROLLUP DETAILED REPORT \n");
	else
		fprintf (fout, ".E STANDARD COST ROLLUP SUMMARY REPORT \n");
	...
	fprintf (fout, ".B1\n");
	fprintf (fout, "NOTE : This report will be accurate only if the BOM and Routing have not been changed since the last Std Cost Rollup.\n");
}
