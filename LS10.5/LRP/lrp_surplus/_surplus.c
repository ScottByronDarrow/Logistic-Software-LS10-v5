...
|=====================================================================|
|  Program Name  : (sk_surplus.c   )                                  |
|  Program Desc  : (Stock Surplus Reporting.                    )     |
|                  (                                              )   |
|---------------------------------------------------------------------|
...
|                                                                     |
| $Log: _surplus.c,v $
| Revision 5.4  2002/11/25 03:16:40  scott
| Updated to use chk_env instead of get_env when applicable.
|
| Revision 5.3  2002/07/17 09:57:23  scott
| Updated to change argument to get_lpno from (1) to (0)
|
| Revision 5.2  2001/08/09 09:30:01  scott
| Updated to add FinishProgram () function
|
| Revision 5.1  2001/08/06 23:27:52  scott
| RELEASE 5.0
|
| Revision 5.0  2001/06/19 08:07:43  robert
| LS10-5.0 New Release as of 19 JUNE 2001
|
| Revision 4.0  2001/03/09 02:28:53  scott
...
| <after Rel-10102000>
|
| Revision 2.0  2000/07/15 08:58:50  gerry
| Forced Revision No. Start 2.0 Rel-15072000
|
| Revision 1.23  2000/07/10 01:52:32  scott
| Updated to replace "@ (" with "@(" to ensure psl_what works correctly
|
| Revision 1.22  2000/06/13 05:29:07  scott
| S/C LSANZ 16400
...
|
| Revision 1.21  2000/01/28 09:31:11  ramon
| Added a save_rec () function call for the headings.
|
| Revision 1.20  2000/01/28 04:11:35  ramon
| For GVision compatibility, I added description fields to separate the fields to be inputted to its description.
|
| Revision 1.19  2000/01/20 07:27:10  scott
| Updated to move CloseDB to be after ML to ensure open_db error does not occur.
| S/C IKHK-2214
|
| Revision 1.18  1999/12/06 01:34:22  scott
| Updated to change CTime to SystemTime due to conflict with VisualC++.
|
| Revision 1.17  1999/11/17 06:40:16  scott
| Updated to remove PNAME as now available with Ctrl-P.
|
| Revision 1.16  1999/11/03 00:22:17  scott
| Updated to change environment FF_ to LRP_
|
| Revision 1.15  1999/10/27 07:33:03  scott
...
|
| Revision 1.13  1999/09/29 10:10:52  scott
| Updated to be consistant on function names.
|
| Revision 1.12  1999/09/17 07:26:45  scott
| Updated for datejul, ctime, ttod, copyright, juldate
| Updated to DateToString, SystemTime, TimeHHMM, StringToDate etc.
|
| Revision 1.11  1999/09/17 00:09:29  scott
| Updated from changes made pre-ansi.
|
| Revision 1.10  1999/09/16 09:20:48  scott
| Updated from Ansi Project
|
| Revision 1.8  1999/06/22 09:20:14  scott
...
|
| Revision 1.7  1999/06/15 07:27:06  scott
| Updated to add log file.
|
=====================================================================*/
#define	CCMAIN
char	*PNAME = "$RCSfile: _surplus.c,v $";
char	*PROG_VERSION = "@(#) - $Header: /usr/LS10/REPOSITORY/LS10.5/LRP/lrp_surplus/_surplus.c,v 5.4 2002/11/25 03:16:40 scott Exp $";
...
void 	CalculateDemad 		(int, int);
int 	CalculateErratic 	(int);
int 	GetProcessMonth 	(int);
void 	PrintSurplus 		(void);
 ...
 char*  argv [])
{
	...
	if (argc != 1 && argc != 11)
	{
		...
		print_at (0,0, "Usage :-\n");
		print_at (1,0, "%s S(umry)/D(etail) <W(arehouse) C(ompany)> <LPNO> <# months> <start group> <end_group> <Valid Methods> <abcCodes> <History Months> <Demand type Included>\n", argv [0]);
        return (EXIT_FAILURE);
	}
	...
	strcpy (programName, ML(" Stock Surplus Report "));
	...
	sprintf (local_rec.sGroup, "%1.1s%-11.11s", local_rec.sClass,
						    local_rec.sCategory);

	sprintf (local_rec.eGroup, "%1.1s%-11.11s", local_rec.eClass, 
						    local_rec.eCategory);

	strcpy (local_rec.demandInclude, "1");

	if (local_rec.TransfersOK [0] == 'Y')
		strcat (local_rec.demandInclude, "2");

	if (local_rec.LostSalesOK [0] == 'Y')
		strcat (local_rec.demandInclude, "3");

	if (local_rec.pcIssues [0] == 'Y')
		strcat (local_rec.demandInclude, "4");
...
ProcessGutz (void)
{
	...
	dsp_screen ("LRP - Future Stock-surplus Report",
									comm_rec.co_no,
									comm_rec.co_name);

	abc_selfield (incc, (wh_flag) ? "incc_id_no" : "incc_hhbr_hash");
	...
	while (!cc && !strcmp (inmr_rec.co_no, comm_rec.co_no))
	{
		...
		wrk_portion 	= j + 1;
		wrk_portion 	*= 4.348;
		wrk_portion 	= covr_prd - wrk_portion;
		if (wrk_portion < 1.0)
			wrk_portion = 0.0;
		wrk_qty			= (qty_ord [j + 1] * wrk_portion);
		wrk_qty			/= 4.348;
		qty_avail		+= wrk_qty;
		wrk_qty			= (qty_proj [j + 1] * wrk_portion);
		wrk_qty			/= 4.348;
		demand			+= wrk_qty;

		sur_qty = qty_avail - demand;
		if (sur_qty > 0.00)
			PrintSurplus ();
 ...
 long   HHBR_HASH)
{
	...
	cc = find_hash (incc, &incc_rec, GTEQ, "r", HHBR_HASH);
	while (!cc && incc_rec.hhbr_hash == HHBR_HASH)
	{
		demand += incc_rec.wks_demand;
		cc = find_hash (incc, &incc_rec, NEXT, "r", HHBR_HASH);
	}
	return (demand);
}
/*===============================
 ...
 int    method)
{
	...
	------------------------------------------*/
	if (!envLrpDmndNeg && store [indx].f_wks_demand < 0.00)
...
PrintSurplus (void)
{
	...
	fprintf (fout, "| %5.0f  %5.0f  %5.0f  %5.0f ", lead_time, sfty_prd,
							   revw_prd, covr_prd);
	fprintf (fout, "|%10.2f %9.2f ", qty_avail, demand);
	fprintf (fout, "|%11.2f", sur_qty);
	fprintf (fout, " %8.2f |\n",(wks_demand) ? sur_qty / wks_demand : 0.00);
...
OpenReport (void)
{
	...
	fprintf (fout, ".PL60\n");
	fprintf (fout, ".L158\n");
	fprintf (fout, ".EFUTURE STOCK-SURPLUS REPORT\n");
