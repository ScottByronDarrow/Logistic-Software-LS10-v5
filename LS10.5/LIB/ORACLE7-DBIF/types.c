#ident	"$Id: types.c,v 5.1 2001/06/21 08:01:02 cha Exp $"
/*
...
//printf("DATE %d %d %d\n", ora_date.century, ora_date.month,
//       ora_date.year );
		*(Date *) appbuf =
			DMYToDate (
				ora_date.day,
...
//printf("Appbuf DATE %ld\n", *(Date *) appbuf );
		break;

	case OT_Serial:
	case OT_Number:
		*(long *) appbuf = isnull ? 0 : *(long *) col -> data;
//printf("\nNUMBER %s : %ld\n", col -> name, *(long *)col -> data);
		break;

	case OT_Money:
//printf("\n%s : %f\n", col -> name, *(float *)col -> data);
	case OT_Double:
		*(double *) appbuf = isnull ? 0.0 : *(double *) col -> data;
//printf("\nDOUBLE %s : %g\n", col -> name,*(double *) col -> data); 
		break;
